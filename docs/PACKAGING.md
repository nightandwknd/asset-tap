...
**Changelog:** Release notes are generated by [git-cliff](https://git-cliff.org/) using Conventional Commits (config: `cliff.toml`). New versions are prepended onto `CHANGELOG.md`; existing entries are not regenerated from git. The squash-body rule lives in [CONTRIBUTING.md](../CONTRIBUTING.md). `scripts/check-changelog-prepend.sh` (CI) proves prepend keeps the header and the previous version. The `[skip ci]` flag prevents the release commit from re-triggering the workflow.
