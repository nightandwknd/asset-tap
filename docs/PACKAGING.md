...
1. **Rust toolchain** (version in [`rust-toolchain.toml`](../rust-toolchain.toml); rustup selects it automatically)
...
**Changelog:** Release notes are generated by [git-cliff](https://git-cliff.org/) using Conventional Commits (config: `cliff.toml`). New versions are prepended onto `CHANGELOG.md`; existing entries are not regenerated from git. GitHub squash uses commit messages, not the PR body; the rule lives in [CONTRIBUTING.md](../CONTRIBUTING.md). `scripts/check-changelog-prepend.sh` (CI) proves prepend keeps the header and the previous version. The `[skip ci]` flag prevents the release commit from re-triggering the workflow.
...
2. Stamps `Cargo.toml`
3. Prepends this version onto `CHANGELOG.md` with git-cliff
4. Creates a release commit and tag
5. Builds all 3 platforms from the tagged commit (macOS universal, Linux, Windows)
6. Creates a GitHub Release with all artifacts and changelog
