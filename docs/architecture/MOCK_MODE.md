...
The demo assets are read from disk at runtime via a path baked in at compile
time (`env!("CARGO_MANIFEST_DIR")`), so they resolve only on the machine that
built the binary. Released binaries ship with mock mode enabled but have no
repo checkout, so `SampleFiles` falls back to small embedded placeholders (a
solid-color PNG and a unit-cube GLB, ~1 KB combined) instead of panicking.
Setting `ASSET_TAP_MOCK_EMBEDDED=1` forces the embedded fallback; the shell
suite uses it to exercise the released-binary path from a checkout, where the
disk assets would otherwise always win.
