...
Grab your platform's installer from the [Download](@/download.md) page.
...
The app carries its own copy of the CLI internally. To use `asset-tap` from your terminal, run the [CLI install](#install-the-cli-macos-linux) below. It is the same binary, kept current independently of the app.
...
For the terminal, run the [Windows CLI install](#install-the-cli-windows) below.
...
The AppImage is the GUI only; pair it with the [CLI install](#install-the-cli-macos-linux) below for terminal use.
...
The script is [readable on GitHub](https://github.com/nightandwknd/asset-tap/blob/main/site/static/install.sh), and every step it performs can be done by hand with the archives on the [Download](@/download.md) page.
