...
Both workflows use a shared composite action (`.github/actions/build-and-package/`) that encapsulates the full build + package pipeline per platform, ensuring CI and Release follow identical code paths.

- **CI** (`.github/workflows/ci.yaml`, PRs only): Layer 0 runs fmt, clippy, check, test, docs, audit, udeps, version-preview in parallel. Layer 1 builds + packages all 3 platforms (macOS native, Linux, Windows) after check passes — installer artifacts uploaded with `-pr-{N}` suffix (e.g., `asset-tap-macos-pr-7`), plus Linux binaries for CLI tests. Layer 2 runs CLI tests using the Linux binary artifact.
- **Release** (`.github/workflows/release.yaml`, push to main): CalVer versioning → release commit (stamps `Cargo.toml` version + generates `CHANGELOG.md`) → tag → parallel build + package for all 3 platforms (macOS universal via `lipo`, Linux .deb/AppImage, Windows NSIS) from the tagged commit → GitHub Release with git-cliff changelog.

**Changelog:** Generated by [git-cliff](https://git-cliff.org/) from Conventional Commits. Config in `cliff.toml`. Release notes are grouped by type (Features, Bug Fixes, etc.) with merge commits and noise filtered out. The release commit includes both the stamped version in `Cargo.toml` and the updated `CHANGELOG.md`, so the tagged commit is the canonical source of truth.
