...
**Asset Tap is a game asset generation pipeline: prompt, image, or mesh in; concept art, textured GLB, humanoid rig, and animation clips out, in one bundle.**
...
**`allow_unset:` (select only):** A dropdown only writes one of its `options`, so a `select` parameter has no way to express "no value" — adding `''` to the options list sends `""` rather than omitting the field. Set `allow_unset: true` to add an explicit `(unset)` entry that writes null, so the user can ask for the provider's own default. `input` widgets (including `type: string` with `widget: input`) already clear to null when emptied. It is **not** how mutual exclusion is expressed — see below — but the two coexist: `aspect_ratio` declares the Multi-View conflict and stays manually clearable. `mutually_exclusive_select_params_are_clearable` in [core/tests/integration_tests.rs](core/tests/integration_tests.rs) enforces both on the Meshy pair.

**Conditional parameters (`requires:` / `conflicts_with:`):** Two optional maps from a sibling parameter's name to the value that triggers the condition. `requires: { auto_size: true }` says the parameter only applies when that sibling holds that value (all listed siblings must); `conflicts_with: { generate_multi_view: true }` says it's invalid when any listed sibling does. A listed `null` means "any non-null value". Declaring a conflict on **one** side is enough — [core/src/providers/conditions.rs](core/src/providers/conditions.rs) evaluates it symmetrically, in declaration order, to a fixed point so chains resolve. References are validated at YAML load: the sibling must be declared on the same model, and no self-reference. Outcome depends on who set the value: **explicitly set** (CLI `--param`, a GUI widget, MCP `params`) and unsatisfied → usage error, exit 2 before the `start` event, like an out-of-range value; **left at the YAML default** → the key is dropped from the request exactly as a null is (and from the bundle's recorded params), the GUI greys the widget out with the reason on hover, and the drop is logged at info. Meshy is why: `decimation_mode` needs `should_remesh`, `origin_at` needs `auto_size`, `target_polycount` is ignored once `decimation_mode` is set, and `aspect_ratio` is rejected alongside `generate_multi_view`.
...
- `V6_ONLY` — `texture_resolution`, `remove_lighting`, `image_enhancement`: the knobs `meshy/v6-lite/image-to-3d` (ai_model `meshy-6-lite`) must **not** carry, so 6-lite is v6's surface minus exactly this set. Each is excluded for its own documented reason: `texture_resolution` is value-level — "the 4k and 8k options are unavailable with meshy-6-lite", so the parameter as declared can't be honoured; `image_enhancement` is gated to "meshy-6, meshy-7.1, or latest"; `remove_lighting` is "meshy-6 only".
- `NOT_IN_V7` / `V7_ONLY` — v7 (native `meshy/v7/image-to-3d`, ai_model `meshy-7.1`, **and** fal's partner-namespace wrapper) is v6's surface minus `remove_lighting` (documented "meshy-6 only"), plus `geometry_resolution` ("Requires meshy-7.1 or latest"), which supersedes the deprecated `ultra_mode`. `geometry_resolution` is the one knob whose _options_ legitimately differ per provider — native offers `standard/2k/4k`, fal's wrapper only `standard/2k` — so parity compares the parameter's presence while `meshy_v7_geometry_resolution_options_follow_each_schema` pins each provider's option list to its own schema.
- `symmetry_mode` is **not** a declared parameter on any model: Meshy deprecated it API-wide ("This parameter no longer affects output"). Where a schema still accepts it, it survives only as an inert body key, never as a knob.

**Per-model, not per-provider.** Aspect ratios are the trap: Meshy's `gpt-image-2` takes the seven-value superset `1:1/16:9/9:16/4:3/3:4/3:2/2:3` while the nano-banana family takes only the first five, so `gpt-image-2` deliberately does **not** reuse the `x-meshy-t2i-params` anchor. Note the direction: sharing the anchor would _under_-advertise `gpt-image-2` rather than advertise values the API rejects — but the fix is the same, an own parameter list. Before extending a shared anchor, confirm every model aliasing it supports the values.

**Verify against provider docs, not against sibling YAML.** Check every parameter against the provider's own API reference. Copying a knob from a neighboring model because it looks similar leads to advertising fields the API rejects. For fal, `scripts/audit-fal-schemas.sh` (`make audit-providers`) does this mechanically, diffing every `providers/fal-ai.yaml` model against fal's live per-endpoint OpenAPI schema — see [docs/architecture/PROVIDERS.md](docs/architecture/PROVIDERS.md) § Auditing against provider schemas. Deliberate skips (knobs out of pipeline scope, deliberate narrowings) are recorded in `scripts/audit-fal-allowlist.json` beside it, each with a note; anything not listed there is a finding.
...
**`image.png` always contains real PNG bytes.** Providers serve other formats (Meshy's text-to-image returns JPEG), so `png_reencode()` in [core/src/images.rs](core/src/images.rs) re-encodes non-PNG bytes before the write. Two things depend on this: the fixed `.png` filename, and the data-URI fallback that hardcodes `data:image/png;base64,` when feeding the 3D stage. **Two strictnesses, on purpose.** In the _pipeline_ a failed re-encode is non-fatal — the original bytes are kept and a warning logged, since a usable image beats discarding a paid generation. _Import and attach_ use `images::to_png()` instead, which refuses bytes it cannot turn into a real PNG: there is no paid generation to salvage, the user handed us the file and can hand us another, and writing junk to `image.png` would break the contract the fixed filename rests on. Both wrap `png_reencode()`; don't collapse them.
...
- **Dev/mock mode:** Read from disk at runtime via `env!("CARGO_MANIFEST_DIR")` (in `SampleFiles` for mock server responses). That path is baked in at compile time, so it resolves only on the build machine — released binaries (which ship with mock enabled) next try the newest downloaded demo bundle (`demo_version` in `bundle.json`) in `ASSET_TAP_MOCK_DEMO_DIR` (if set) then the output directory, then fall back to small embedded placeholders in `core/src/api/mock/assets/`. `ASSET_TAP_MOCK_EMBEDDED=1` skips all disk tiers for testing.
...
**Clip packs:** The Quaternius Standard libraries live in `packs/ual{1,2}/pack.glb` (trimmed animation GLBs only — no mannequin, no `_RM`, no Source). They are **never compiled into the binary**. The release workflow zips them as `clip-packs.zip` and writes `clip-packs-manifest.json` with a SHA-256. `asset-tap clip download` / Help / Animate → Add pack → Download Universal Animation Libraries fetch that archive, verify the hash, and install missing ids into `packs_root()`. Already-installed ids are skipped so a user Source upgrade is not overwritten; `--force` refreshes only packs stamped by a previous download. MCP `clip_download` is the same document as `--json clip download`. Debug builds and tests can set `ASSET_TAP_CLIP_PACKS_DIR` (or, in debug, read `packs/` next to the source tree) to skip GitHub; `ASSET_TAP_CLIP_PACKS_MANIFEST_URL` / `ASSET_TAP_CLIP_PACKS_URL` override the release URLs (and skip the local dir).

**Machine-interface fixtures:** the golden fixtures in `cli/tests/fixtures/machine-interface/` are attached to each release the same way — `machine-interface-fixtures.zip` plus a `machine-interface-fixtures-manifest.json` carrying the interface version and SHA-256 — built by `scripts/machine-interface-fixtures.sh` (`make fixtures-bundle`) so downstream consumers fetch and verify them instead of copying from a sibling checkout.

**Bundle import/export:** Bundles can be exported as `.zip` archives and imported via File > Import Bundle, the bundle info button, or drag-and-drop onto **zones** (not the whole window). `import_bundle()` accepts a zip, a bundle folder, a lone `.glb`, or an image. The candidate walk stops at `MAX_IMPORT_DEPTH` and skips any subdirectory that `looks_like_bundle`, so dropping a folder **of** bundles is refused rather than collapsed into one; once something is promoted, the runners-up are deleted from the new bundle and emptied directories pruned, keeping the standard-filenames contract. Loose files (and archives that use non-standard names) are wrapped into a timestamped bundle: renamed to `model.glb` / `image.png`, textures extracted from the GLB when missing, JPEG/WebP/etc. re-encoded to PNG, and a v2 `bundle.json` written with an `import` op. Drop zones: sidebar image slot → pipeline input; Bundle Info pane → always a new bundle; empty Image/3D tab (or the Add picker) → `attach_to_bundle()` (which fills the empty slot — the GUI only offers attach on an empty Image/3D tab; core will also replace a filled slot and clear the outgoing model's textures — refuses an undecodable still, and **merges** provenance rather than restamping a generated bundle as an import); Animation panel → clip-pack install. `import_loose_files()` pairs a still and a mesh dropped together on Bundle Info. `looks_like_clip_pack()` classifies a UAL zip so it is not wrapped as `model.glb` — packs dropped on the preview toast to use the Animation panel. Classification is **name plus corroborating content**, never name alone: only a shape that can hold clips is considered (a dir, `.zip`, or `GLTF_EXTS`), **strong** tokens (`universal animation`, `clip-packs`, `unreal-godot`, a `ual1`/`ual2` token — on the filename or its immediate parent, never a distant ancestor) stand on their own, and **weak** ones (`pack`, `*_standard`, `*_source`) need `MIN_PACK_CLIPS` animations in the mesh, in the folder (bounded by `MAX_CLASSIFY_CANDIDATES`, since this runs on the UI thread during a hover), or `MIN_PACK_GLTF_ENTRIES` glTF entries in the archive. Without that, a user's `hero_Source.glb` or `armor_standard.png` was refused as a pack with no way to import it. The GUI memoizes the verdict per path for the length of a drag (`App::is_pack_drop_cached`) — classifying a zip parses its central directory, which the hover path would otherwise redo every frame. The `extract_zip_to_dir()` helper handles zip import and demo download, auto-detecting and stripping a common top-level directory prefix while preserving subdirectory structure (e.g., `textures/`).

**Zones only work because of `gui/src/dnd.rs`.** winit 0.30 reports a file drag as `HoveredFile`/`DroppedFile` with **no coordinates** and emits no `CursorMoved` for the whole drag, so `pointer_latest_pos()` is stale (usually `None`, since the pointer left the window to pick the file up) and every zone's rect test is `false` — drops look silently dead. `App::raw_input_hook` queries the OS cursor (CoreGraphics `CGEventGetLocation` on macOS, `GetCursorPos` on Windows) and injects a synthetic `PointerMoved`, after which ordinary `rect_contains_pointer`/`Response::hovered` work during a drag. Never gate a drop zone on pointer position without that hook in place. `App::drop_unclaimed` runs at the end of `ui()` on whatever no zone took, so a drop is never silently swallowed: with a pointer it toasts the zone the file belongs to, and with **no** pointer — X11 reports no drag coordinates, Wayland no file drops, or the query failed — it reads that absence as "zones are unavailable" and routes by file type (still → pipeline input, pack → install, else new bundle), which is what shipped before zones existed. A zone that can't use what was dropped must claim nothing (every zone goes through `App::drop_zone`, which claims only the files it uses, and claims nothing while `App::modal_open()`), or `drop_unclaimed` never sees it. Upstream: [egui#4655](https://github.com/emilk/egui/issues/4655).
...
- `CHANGELOG.md` - Rolling changelog (git-cliff prepends each release; historical entries are not regenerated)
...
- **`interface` field is a `"MAJOR.MINOR"` string** (e.g. `"1.1"`), Terraform `format_version`-style: MAJOR bumps on breaking wire changes (consumers must reject an unrecognized MAJOR), MINOR bumps on additive/backward-compatible changes (consumers ignore unknown fields, tolerate a higher MINOR). Single-sourced from `machine::INTERFACE_VERSION`. See [docs/CLI_MACHINE_INTERFACE.md](docs/CLI_MACHINE_INTERFACE.md)'s Versioning section.
- **`--version --json`** emits `{"version":"<calver>","interface":"1.1"}` instead of the plain human version line. Detected on raw `std::env::args()` in `main()` _before_ `Cli::parse()`, because clap's derived `#[command(version)]` handles bare `--version` and exits before application code runs. Plain `--version` (no `--json`) is untouched.
- **`--install PATH`** copies a finished run's primary artifact (`model.glb`, or `image.png` under `--image-only`) to PATH, in addition to the bundle. Deliberately **wire-silent**: the `result` event is unchanged (the caller supplied the path) and the copy is logged on stderr. A `.png`/`.glb` mismatch against the stage the run will execute is a usage error raised before `start`, so a typo costs no API call.
