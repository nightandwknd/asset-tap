...
**Dependabot** (`.github/dependabot.yml`): Cargo updates daily at noon CST, GitHub Actions weekly. Uses `lockfile-only` versioning to avoid `Cargo.toml` churn. The entire 3D rendering stack (three-d, three-d-asset, egui, eframe, egui_extras, egui-phosphor, glow) is ignored — these are version-locked for compatibility and must be upgraded together manually (see §10 above). Minor+patch updates are grouped by ecosystem (serde, tokio, tracing, catch-all); major bumps surface as individual PRs.
