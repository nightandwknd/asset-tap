...
**Changelog:** Generated by [git-cliff](https://git-cliff.org/) from Conventional Commits. Config in `cliff.toml`. The squash body is the release note: write it for people who use the app. New versions are prepended onto `CHANGELOG.md`; existing entries are not regenerated from git.
