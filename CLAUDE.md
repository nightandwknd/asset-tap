...
- **Dev/mock mode:** Read from disk at runtime via `env!("CARGO_MANIFEST_DIR")` (in `SampleFiles` for mock server responses). That path is baked in at compile time, so it resolves only on the build machine — released binaries (which ship with mock enabled) next try the newest downloaded demo bundle in the output directory (`demo_version` in `bundle.json`), then fall back to small embedded placeholders in `core/src/api/mock/assets/`. `ASSET_TAP_MOCK_EMBEDDED=1` skips both disk tiers for testing.
